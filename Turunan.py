...
class Teori(MataKuliah):
    ...
    def tampilkanInformasi(self):
        ...
        print(f"Mata Kuliah  : {self._nama_mk} ({self._get_jumlah_sks()} SKS)")
        print(f"Jam          : {self._get_jam()} | Ruang: {self._get_ruangan()}")
        print(f"Kelas        : {self._kelas_paralel}")
        print(f"Dosen        : {self._get_info_dosen()}")
...
def sistem():
    ...
    while True:
        ...
        print("5. Input Jadwal (Nama MK, Jam, Ruangan) & SIMPAN")
